...
class VolumeAttachmentCommands(object):
    ...
    def remove(
        ...
        to delete the entry from the block_device_mapping table.

        Usage of this command should be reserved for situations where the volume
        attachment record only exists in Nova, and Cinder does not have a
        corresponding entry. As such, there is no need to propagate the deletion
        to Cinder.
        ...
        try:
            ...
            with context.target_cell(ctxt, im.cell_mapping) as cctxt:
                ...
                with locked_instance(im.cell_mapping, instance, locking_reason):
                    ...
                    deleted_bdm = objects.BlockDeviceMapping.get_by_volume_and_instance(
                        cctxt, volume_id, instance_uuid)
                    LOG.info(
                        f"The updated BDM record after destruction: {deleted_bdm}"
                    )
        except exception.VolumeBDMNotFound as e:
            print(str(e))
            return 3
